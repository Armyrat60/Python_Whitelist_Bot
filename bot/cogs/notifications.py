...
from bot.config import WHITELIST_TYPES, GUILD_ID, log
...
class NotificationsCog(commands.Cog):
    ...
    async def expiry_check(self):
        ...
        retention = int(await self.bot.db.get_setting("retention_days", "90"))
        ...
        for wt in WHITELIST_TYPES:
            ...
            for days_before in (7, 1):
                ...
                rows = await self.bot.db.fetchall(
                    ...
                    WHERE whitelist_type=%s AND status <> 'active'
                    AND updated_at >= %s AND updated_at < %s
                    """,
                    (wt, cutoff_start, cutoff_end),
                )
